...
impl NotesEngine {
    ...
    pub fn write_note(&self, note: &Note) -> Result<String> {
        ...
        hash_cmd.args(["hash-object", "-w", "--stdin"])
            .stdin(Stdio::piped())
            .stdout(Stdio::piped());
        ...
        let tree_cmd = self.git_cmd()
            .args(["ls-tree", &ref_path])
            .output()?;
        ...
        commit_cmd.args(["commit-tree", &new_tree_hash, "-m", &format!("Update note {}", note.id)]);
        
        // Find parent commit if ref exists
        let rev_parse = self.git_cmd().args(["rev-parse", "-q", "--verify", &ref_path]).output()?;
    ...
    pub fn delete_note(&self, note_id: Uuid, namespace: &Namespace) -> Result<()> {
        ...
        if !tree_cmd.status.success() {
            return Err(GnError::NotFound(format!("Namespace {} not found", ref_path)));
        }
        ...
        let mut mktree_cmd = self.git_cmd();
        mktree_cmd.arg("mktree").stdin(Stdio::piped()).stdout(Stdio::piped());
        let mut child = mktree_cmd.spawn()?;
        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(new_tree_input.as_bytes())?;
        }
        ...
        let mut commit_cmd = self.git_cmd();
        commit_cmd.args(["commit-tree", &new_tree_hash, "-m", &format!("Delete note {}", note_id)]);
    ...
    pub fn list_notes_for_commit(&self, commit: &str, namespace: &Namespace) -> Result<Vec<Note>> {
        ...
        let filtered = all_notes.into_iter().filter(|n| n.commit == commit).collect();
        Ok(filtered)
    }
}
