use clap::Args;
use gn_core::{NotesEngine, NoteStatus};
...
pub fn run(args: &ResolveArgs) -> Result<()> {
    ...
    let status = match args.status.to_lowercase().as_str() {
        ...
        _ => return Err(anyhow!("Invalid status. Use approved, rejected, resolved, or open.")),
    };
    ...
    println!("✓ Note {} marked as {}", note.id, args.status.to_lowercase());
